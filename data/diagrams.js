...
window.PORTFOLIO_DIAGRAMS = {
  aws: {
    ...
    nodes: [
      ...
      { kind: 'store', label: { en: 'S3', es: 'S3' }, sub: { en: 'datasets · models', es: 'datasets · modelos' } },
      { kind: 'svc',   label: { en: 'EC2', es: 'EC2' }, sub: { en: 'app + classifier', es: 'app + clasificador' } },
  ...
  spark: {
    ...
    nodes: [
      ...
      { kind: 'ml',    label: { en: 'ML training', es: 'Entrenamiento ML' }, sub: { en: 'features → model', es: 'features → modelo' } },
    ],
    note: {
      ...
      es: 'El patrón que aplico con Spark / Databricks: transformaciones distribuidas por lotes que terminan en Parquet columnar antes de la ingeniería de features y el entrenamiento.',
    },
  },
  docker: {
    title: { en: 'Containerized ML pipeline · DCE-MRI thesis (simplified)', es: 'Pipeline ML containerizado · tesis DCE-MRI (simplificado)' },
    nodes: [
      ...
      { kind: 'ml',    label: { en: 'Model runtime', es: 'Runtime del modelo' }, sub: { en: 'PyTorch', es: 'PyTorch' } },
    ...
    note: {
      ...
      es: 'Modelo, API REST y runtime empaquetados juntos para ejecuciones reproducibles. Vista genérica — sin hosts, puertos ni rutas internas.',
    },
  },
};
