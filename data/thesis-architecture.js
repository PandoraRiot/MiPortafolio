...
window.THESIS_ARCH = {
  ...
  training: {
    phase1: {
      ...
      es: 'Fase 1 · 10 épocas · backbone congelado, solo entrena la cabeza (AdamW, LR 1e-3*)',
    },
    phase2: {
      en: 'Phase 2 · 20 epochs · partial unfreeze of the top blocks + head (LR 1e-4*, cosine annealing)',
      es: 'Fase 2 · 20 épocas · descongelado parcial de los bloques superiores + cabeza (LR 1e-4*, cosine annealing)',
    },
    note: {
      en: '* Values from the repo\'s final config (VF). EfficientNet-B3 uses 5e-4 / 5e-5. Loss: focal loss (α 0.6, γ 2.0).',
      es: '* Valores de la configuración final del repo (VF). EfficientNet-B3 usa 5e-4 / 5e-5. Pérdida: focal loss (α 0.6, γ 2.0).',
    },
  },
  models: [
    {
      ...
      blocks: [
        { id: 'stem', label: 'stem', kind: 'conv', state: 'frozen', shape: '16 × 128²', params: '0.5K',
          detail: { en: '3×3 strided convolution — first low-level edges and textures.', es: 'Convolución 3×3 con stride — primeros bordes y texturas.' } },
        { id: 'stages.0', label: 'stages.0', kind: 'conv', state: 'frozen', shape: '32 × 128²', params: '4K',
          detail: { en: '1 × MobileNetV2 inverted-residual block.', es: '1 × bloque inverted-residual MobileNetV2.' } },
        { id: 'stages.1', label: 'stages.1', kind: 'conv', state: 'frozen', shape: '64 × 64²', params: '87K',
          detail: { en: '3 × MobileNetV2 blocks — local features, downsampling.', es: '3 × bloques MobileNetV2 — features locales y downsampling.' } },
        { id: 'stages.2', label: 'stages.2', kind: 'transformer', state: 'frozen', shape: '96 × 32²', params: '0.66M',
          detail: { en: 'MV2 + MobileViT block: local conv → unfold into patches → 2 Transformer layers (d=144) → fold → fuse with the conv path.', es: 'MV2 + bloque MobileViT: conv local → unfold en parches → 2 capas Transformer (d=144) → fold → fusión con la rama convolucional.' } },
        ...
        { id: 'head', label: 'head', kind: 'head', state: 'head', shape: 'benign · malignant', params: '—',
          detail: { en: 'Global pooling + new classifier (benign vs. malignant). Trains in both phases.', es: 'Pooling global + clasificador nuevo (benigno vs. maligno). Entrena en ambas fases.' } },
      ],
    },
    {
      id: 'resnet50',
      ...
      blocks: [
        { id: 'conv1', label: 'conv1', kind: 'conv', state: 'frozen', shape: '64 × 128²', params: '9K',
          detail: { en: '7×7 convolution + BN + max-pool.', es: 'Convolución 7×7 + BN + max-pool.' } },
        { id: 'layer1', label: 'layer1', kind: 'conv', state: 'frozen', shape: '256 × 64²', params: '0.22M',
          detail: { en: '3 bottleneck residual blocks.', es: '3 bloques residuales bottleneck.' } },
        { id: 'layer2', label: 'layer2', kind: 'conv', state: 'frozen', shape: '512 × 32²', params: '1.22M',
          detail: { en: '4 bottleneck residual blocks.', es: '4 bloques residuales bottleneck.' } },
        { id: 'layer3', label: 'layer3', kind: 'conv', state: 'frozen', shape: '1024 × 16²', params: '7.10M',
          detail: { en: '6 bottleneck residual blocks.', es: '6 bloques residuales bottleneck.' } },
        { id: 'layer4', label: 'layer4', kind: 'conv', state: 'unfrozen', shape: '2048 × 8²', params: '14.96M',
          detail: { en: '3 bottleneck blocks. Unfrozen in phase 2 — also the Grad-CAM target layer (layer4[-1]).', es: '3 bloques bottleneck. Descongelado en la fase 2 — también es la capa objetivo de Grad-CAM (layer4[-1]).' } },
        { id: 'fc', label: 'fc', kind: 'head', state: 'head', shape: 'benign · malignant', params: '—',
    ...
    },
    {
      ...
      blocks: [
        { id: 'features.0', label: 'features.0', kind: 'conv', state: 'frozen', shape: '40 ch', params: '1K',
          detail: { en: 'Stem: 3×3 strided convolution.', es: 'Stem: convolución 3×3 con stride.' } },
        ...
        { id: 'classifier', label: 'classifier', kind: 'head', state: 'head', shape: 'benign · malignant', params: '—',
          detail: { en: 'Dropout + replaced linear layer. Trains in both phases.', es: 'Dropout + capa lineal reemplazada. Entrena en ambas fases.' } },
      ],
    },
  ],
};
